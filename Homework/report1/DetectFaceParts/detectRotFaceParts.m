    ...
    if( nargout >= 2 )
        ...
        if( nargout >= 3 )
            ...
            for i=1:size(fourpoints,1)
                ...
                T = maketform('FITGEOTRANS',V,U);
                faces{i,1} = imwarp(X,T,'bicubic','XData',[1,leng(i,1)],'YData',[1,leng(i,1)]);
                bbfaces{i,1} = imwarp(bbX,T,'bicubic','XData',[1,leng(i,1)],'YData',[1,leng(i,1)]);
            end
        end
    end
end
